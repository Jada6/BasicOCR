...
def histogram(image):
    """ Return histogram of the image as dictionary
    representing number of black and white pixels """
...
def analyze_image(image):
    ...
    result = {'proportion': 0,
              'subproportions': [[0 for _ in range(SPLIT_NUMBER)]
                                 for _ in range(SPLIT_NUMBER)]}

    hist = histogram(image)
    result['proportion'] = round(100 * hist['black']
                                 / (image.size[0] * image.size[1])) / 100
    ...
    for x in range(SPLIT_NUMBER):
        for y in range(SPLIT_NUMBER):
            ...
            result['subproportions'][x][y] = \
                min((round(100 * hist['black'] / hist['white']) / 100), 2)
            # why min: proportion bigger then 2 has just I,
            # which proportion is converting to infinity
...
def crop_image(image):
    ...
    return image.crop((extremes['left'], extremes['top'],
                       extremes['right'], extremes['bot']))


def split_image(image):
    """" Return SPLIT_NUMBER^2 subimages (list of lists of images) """
    sub_images = [[[] for _ in range(SPLIT_NUMBER)]
                  for _ in range(SPLIT_NUMBER)]
    ...
    for x in range(SPLIT_NUMBER):
        for y in range(SPLIT_NUMBER):
            coords = (((width / SPLIT_NUMBER) * x),
                      ((height / SPLIT_NUMBER) * y),
                      ((width / SPLIT_NUMBER) * (x + 1)),
                      ((height / SPLIT_NUMBER) * (y + 1)))
